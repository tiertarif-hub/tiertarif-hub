...
const SmartImagePreview = ({ imageUrl, label, initials }: { imageUrl?: string; label: string; initials: string }) => {
  ...
  if (!optimizedImage) {
    return (
      <div className="flex h-full w-full items-center justify-center bg-primary text-5xl font-black text-white">
        {initials}
      </div>
    );
  }
...
const HeroPreview = ({ imageUrl }: { imageUrl?: string }) => {
  ...
  return (
    <div className="relative aspect-[16/7] overflow-hidden rounded-3xl border border-slate-200 bg-primary">
  ...
  return (
    <div className="space-y-8">
      <div className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
        <div className="border-b border-slate-100 tt-admin-hero-card p-6 text-white md:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <div className="mb-3 inline-flex items-center gap-2 rounded-full tt-admin-accent-pill px-3 py-1 text-xs font-black uppercase tracking-[0.18em]">
                <Users className="h-4 w-4 text-secondary" /> Team & Über uns
              </div>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Button type="button" onClick={handleSave} disabled={updateSetting.isPending} className="tt-coral-shine font-black">
        ...
        {hasUnsavedChanges && <div className="border-b tt-admin-soft-alert px-6 py-3 text-sm font-semibold">Ungespeicherte Änderungen vorhanden — bitte speichern, bevor du die Seite verlässt.</div>}
      </div>
      ...
      <div className="grid gap-8 xl:grid-cols-[1fr_380px]">
        <div className="space-y-8">
          <Card className="border-slate-200 bg-white shadow-sm">
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <Switch checked={content.enabled} onCheckedChange={(checked) => updateContent({ enabled: checked })} className="data-[state=checked]:bg-primary" />
              </div>
          ...
          <Card className="border-slate-200 bg-white shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between gap-4">
              ...
              <Button type="button" onClick={addMember} className="tt-teal-shine"><Plus className="mr-2 h-4 w-4" />Person hinzufügen</Button>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid gap-5 md:grid-cols-2">
                    ...
                    <AccordionItem key={member.id} value={member.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 px-0">
                      <AccordionTrigger className="px-5 py-4 hover:no-underline">
                        <div className="flex min-w-0 items-center gap-4 text-left">
                          <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-primary text-sm font-black text-white">
                          ...
                          <div className="space-y-5">
                            <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                              <div className="flex items-center gap-3">
                                <Switch checked={member.is_active} onCheckedChange={(checked) => updateMember(member.id, { is_active: checked })} className="data-[state=checked]:bg-primary" />
        ...
        <aside className="space-y-6 xl:sticky xl:top-24 xl:self-start">
          <Card className="border-slate-200 bg-white shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><ImageIcon className="h-5 w-5 text-secondary" />Vorschau-Status</CardTitle>
            ...
            <CardContent className="space-y-4 text-sm">
              ...
              <div className="flex items-center justify-between rounded-xl bg-slate-50 p-3"><span className="text-slate-600">Aktive Personen</span><Badge className="bg-primary text-white">{content.team_members.filter((member) => member.is_active).length}</Badge></div>
              ...
              <div className="rounded-xl bg-secondary/12 border border-secondary/25 p-4 text-primary"><strong>RLS-Hinweis:</strong> Für öffentliche Anzeige muss die Setting-Key <code>about_page_content</code> öffentlich lesbar sein. Dafür liegt die Migration im ZIP.</div>
              <Button type="button" onClick={handleSave} disabled={updateSetting.isPending} className="w-full tt-coral-shine font-black">
